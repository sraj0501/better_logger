...
11. **Middleware** (`middleware.py`) — `RuntimeNarrativeMiddleware` wraps each request in **`async with story(...)`** (not sync `with`), forwarding `renderers`, `failure_analyzer`, and diagnostic kwargs. Story and failure events use **`emit_async`**. Stage events use **`emit_async`** when the route uses **`async with stage`**; sync **`with stage`** in a route still uses sync **`emit()`** for those stages.
...
Implement a class with `handle(self, event: object) -> None` (sync) or `async def handle(self, event: object) -> None` (async) and pass it to `story(..., renderers=[MyRenderer()])`. No base class or registration required. Async renderers are awaited for story and failure events when using `async with story(...)`, and for stage events when handlers use **`async with stage(...)`** (including per-request stories from `RuntimeNarrativeMiddleware` when routes use async stages). Sync **`with stage(...)`** keeps sync **`emit()`** for stages.
